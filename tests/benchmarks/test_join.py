...
@run_up_to_nthreads("small", 40, reason="Does not finish")
@pytest.mark.client("small")
def test_join_big(client, memory_multiplier, configure_shuffling):
...
@pytest.mark.client("small")
def test_join_big_small(client, memory_multiplier, configure_shuffling):
...
@pytest.mark.client("small")
def test_set_index(client, persist, memory_multiplier, configure_shuffling):
    memory = cluster_memory(client)  # 76.66 GiB
    ...
    wait(df_indexed.size, client, 20 * 60)
