...
@pytest.mark.skip(reason="Only using first 7 queries for now")
@coiled_function()
def test_query_8(connection):
    connection().execute(
        f"""
        with part as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}part/*.parquet')),
             supplier as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}supplier/*.parquet')),
...
@pytest.mark.skip(reason="Only using first 7 queries for now")
@coiled_function()
def test_query_11(connection):
    connection().execute(
        f"""
        with partsupp as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}partsupp/*.parquet')),
             supplier as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}supplier/*.parquet')),
             nation as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}nation/*.parquet'))
...
@pytest.mark.skip(reason="Only using first 7 queries for now")
@coiled_function()
def test_query_15(connection):
    connection().execute(
        f"""
        with recursive supplier as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}supplier/*.parquet')),
             lineitem as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}lineitem/*.parquet')),
             revenue as (
                select
...
@pytest.mark.skip(reason="Only using first 7 queries for now")
@coiled_function()
def test_query_19(connection):
    connection().execute(
        f"""
        with lineitem as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}lineitem/*.parquet')),
             part as (select * from read_parquet('{DATASETS[ENABLED_DATASET]}part/*.parquet'))
