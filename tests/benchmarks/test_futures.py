...
def test_single_future(client):
    """How quickly can we run a simple computation?
    Repeat the test a few times to get a more sensible
    cumulative measure.
    """
    for i in range(100):
        client.submit(inc, i).result()


@run_up_to_nthreads("small", 50, reason="fixed dataset")
@pytest.mark.client("small")
@pytest.mark.parametrize("rootish", ["rootish", "non-rootish"])
def test_large_map(client, rootish):
    """What's the overhead of map these days?"""
    if rootish == "rootish":
        futures = client.map(inc, range(100_000))
    else:

        def inc_with_deps(i, deps):
            return i + 1

        deps = client.map(inc, range(5))
        futures = client.map(inc_with_deps, range(100_000), deps=deps)
...
@pytest.mark.client("small")
def test_large_map_first_work(client):
    """
...
@pytest.mark.client("small")
def test_memory_efficient(client):
    """
    We hope that we pipeline xs->ys->zs without keeping all of the xs in memory
    to start. This may not actually happen today.
    """
    xs = client.map(np.random.random, [20_000_000] * 100, pure=False)
    ys = client.map(slowinc, xs, delay=1)
    zs = client.map(slowdec, ys, delay=1)
