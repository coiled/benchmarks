    ...
    with Cluster(
        ...
        with Client(cluster) as client:  # noqa F841
            ddf = dd.read_parquet(
                ...
                ).repartition(partition_size=partition_size).set_index('DOLocationID')
            ddf = ddf.assign(travel_time = ddf['tpep_dropoff_datetime'] - ddf['tpep_pickup_datetime'])
            ddf2 = ddf.groupby([ddf['tpep_pickup_datetime'].dt.year,
